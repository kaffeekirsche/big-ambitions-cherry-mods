...
namespace ManagedDoom
{
	public sealed class DoomGame
	{
		...
		{
			options.Skill = (GameSkill)MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp((int)skill, (int)GameSkill.Baby, (int)GameSkill.Nightmare);

			if (options.GameMode == GameMode.Retail)
			{
				options.Episode = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp(episode, 1, 4);
			}
			...
			if (options.GameMode == GameMode.Commercial)
			{
				options.Map = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp(map, 1, 32);
			}
			else
			{
				options.Map = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp(map, 1, 9);
			}
