...
namespace MeltySynth
{
    internal sealed class Voice
    {
        ...
        public void Start(RegionPair region, int channel, int key, int velocity)
        {
            ...
            instrumentPan = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp(region.Pan, -50F, 50F);
        ...
        public bool Process()
        {
            ...
            if (dynamicCutoff)
            {
                ...
                smoothedCutoff = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp(newCutoff, lowerLimit, upperLimit);
            ...
            var angle = (MCG_Doom.Compatibility.ManagedDoomNet472Compat.PI / 200F) * (channelInfo.Pan + instrumentPan + 50F);
            if (angle <= 0F)
            {
            ...
            {
                currentMixGainLeft = mixGain * MCG_Doom.Compatibility.ManagedDoomNet472Compat.Cos(angle);
                currentMixGainRight = mixGain * MCG_Doom.Compatibility.ManagedDoomNet472Compat.Sin(angle);
            }

            currentReverbSend = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp(channelInfo.ReverbSend + instrumentReverb, 0F, 1F);
            currentChorusSend = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Clamp(channelInfo.ChorusSend + instrumentChorus, 0F, 1F);
