...
namespace MeltySynth
{
    ...
    {
        private static readonly float resonancePeakOffset = 1 - 1 / MCG_Doom.Compatibility.ManagedDoomNet472Compat.Sqrt(2);
        ...
        public void SetLowPassFilter(float cutoffFrequency, float resonance)
        {
            if (cutoffFrequency < 0.499F * synthesizer.SampleRate)
            {
                ...
                var w = 2 * MCG_Doom.Compatibility.ManagedDoomNet472Compat.PI * cutoffFrequency / synthesizer.SampleRate;
                var cosw = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Cos(w);
                var alpha = MCG_Doom.Compatibility.ManagedDoomNet472Compat.Sin(w) / (2 * q);
