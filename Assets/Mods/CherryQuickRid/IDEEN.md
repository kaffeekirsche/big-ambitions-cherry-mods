...
3. **Fahrgast** – Anfrage wird nach zufälliger Wartezeit (Spielzeit) generiert, der Annehmen/Ablehnen-Dialog erscheint aber erst, wenn das Auto steht oder Schrittgeschwindigkeit fährt (bis dahin zeigt das Panel „Neue Anfrage – anhalten zum Annehmen“). Nach Annahme: zufällige Gebäudetür in Reichweite als Abholpunkt, NPC spawnen (`Characters/HumanDefinitionLow`), Kartenpin, einsteigen im Pickup-Radius, dann Ziel = andere Adresse (min/max Distanz). Fahrtstatus und Adressen liegen in `QuickRidMission`; beim Laden mit laufender Fahrt wird sie abgebrochen (Wiederherstellung = Stufe 5). Noch kein Geld.
